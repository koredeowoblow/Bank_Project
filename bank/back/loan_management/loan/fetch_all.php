...
if (isset($_POST['search']['value'])) {
    ...
    $query .= " WHERE principal like '%" . $search_value . "%' ";
    $query .= " OR id like '%" . $search_value . "%' ";
}
$column_order = array('id', 'user_id', 'principal', 'duration');
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($rows = mysqli_fetch_array($result)) {
                ...
                if ($update == 1) {
                    $status = $row['status'];
                    if ($status == "pending") {
                        $option = '<div  style="float:right"  ><a href="javascript:void()"  id="editbtn"  data-id="' . $row['id'] . '"   class="text-primary"> <i class="fa fa-info-circle "></i>info</a>&nbsp;&nbsp;|<a href="javascript:void()"  id="approvebtn"  data-id="' . $row['id'] . '"   class="text-success"> <i class="fa fa-check-circle "></i>approve</a>&nbsp;&nbsp;|<a href="javascript:void()" data-id="' . $row['id'] . '" class="text-warning rejectbtn" id="rejectbtn"> <i class="fas fa-user-slash"></i>reject</a></div>';
                        # code...
                    }
                     else {
